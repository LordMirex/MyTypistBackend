...
class DocumentBatch(BaseModel):
    ...
    def validate_documents(cls, v):
        for i, doc in enumerate(v):
            if 'title' not in doc:
                raise ValueError(f'Document {i+1} missing required field: title')
            if 'placeholder_data' not in doc:
                raise ValueError(f'Document {i+1} missing required field: placeholder_data')
        return v


class DocumentBatchResponse(BaseModel):
    """Batch document generation response schema"""
    batch_id: str
    total_documents: int
